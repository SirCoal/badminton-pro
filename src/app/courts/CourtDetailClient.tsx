...
// Exact same UI as the demo, wired to your Supabase database.
...
// ─── Placeholder photo panels (same as demo) ──────────────────────────────────
...
export default function CourtDetailClient({ court }: { court: Court }) {
  ...
  // Build photo list — real image first, then decorative panels (same as demo)
  const photos = [
    court.image_url
  ...
  // Fetch slots from Supabase when date changes
  useEffect(() => {
    ...
    <div style={{ minHeight: "100vh", background: "#f5f2ed",
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .tag {
          display: inline-block; border: 1px solid #1c1c1c;
      ...
      <header style={{ borderBottom: "1px solid #d8d5d0", background: "#f5f2ed",
        ...
        <Link href="/" style={{ display: "flex", alignItems: "center", gap: 10,
          ...
          <span className="pfont" style={{ fontSize: 18, fontWeight: 700, letterSpacing: "-0.02em" }}>
            CourtBook
          </span>
        </Link>
        <nav className="sfont" style={{ display: "flex", gap: 32, fontSize: 13,
      ...
      <main style={{ maxWidth: 1200, margin: "0 auto", padding: "48px 40px" }}>
        <div className="main-grid" style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: 48 }}>
          ...
          {/* ── LEFT: Gallery + Info ─────────────────────────────────────── */}
          <div>
            {/* Main photo — real image or styled panel */}
            <div style={{ background: currentPhoto.color, height: 420, position: "relative",
              ...
              {currentPhoto.src ? (
                ...
                <>
                  <div style={{ position: "absolute", border: `2px solid ${currentPhoto.accent}44`,
                    inset: 28, pointerEvents: "none" }} />
                  <div style={{ textAlign: "center", zIndex: 1 }}>
                    <div style={{ fontSize: 72, marginBottom: 12 }}>{currentPhoto.icon}</div>
                    <div className="sfont" style={{ color: currentPhoto.accent, fontSize: 12,
                      letterSpacing: "0.15em", textTransform: "uppercase" }}>{currentPhoto.label}</div>
                  </div>
                </>
              )}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, marginBottom: 32 }}>
                {[
                  ...
                  <div key={s.label} className="stat-box">
                    <div className="sfont" style={{ fontSize: 11, color: "#888",
          ...
          <div>
            ...
            <h1 className="pfont" style={{ fontSize: 40, lineHeight: 1.1, marginBottom: 6 }}>
              {court.name}
            </h1>
            <p className="sfont" style={{ fontSize: 14, color: "#666", marginBottom: 24 }}>
              {court.location}
            </p>
            ...
            <div style={{ background: "#1c1c1c", color: "#f5f2ed", padding: "20px 24px",
              ...
              <span className="pfont" style={{ fontSize: 36, fontWeight: 900 }}>
                ${court.price_per_hour ?? "—"}
              </span>
              <span className="sfont" style={{ fontSize: 14, opacity: 0.6, letterSpacing: "0.06em" }}>
                / HOUR
              </span>
            </div>
            ...
            <div style={{ marginBottom: 28 }}>
                ...
                <div style={{ display: "flex", gap: 4 }}>
                  <button onClick={() => setWeekOffset(w => Math.max(0, w - 1))}
                    style={{ background: "none", border: "1px solid #ccc", width: 30, height: 30,
                      ...
                      fontSize: 16, opacity: weekOffset === 0 ? 0.3 : 1 }}>‹</button>
                ...
                {weekDates.map(d => {
                  ...
                  return (
                    <button key={d.toISOString()}
                      ...
                      <span className="sfont" style={{ fontSize: 10, letterSpacing: "0.08em",
                        opacity: isSelected ? 1 : 0.5 }}>{DAYS[d.getDay()]}</span>
                      <span className="pfont" style={{ fontSize: 20, fontWeight: 700, lineHeight: 1.2 }}>
                        {d.getDate()}
                      </span>
                      {isTodayDate && (
                        <span className="sfont" style={{ fontSize: 8,
                          color: isSelected ? "#a8e0c0" : "#2d7a52", letterSpacing: "0.1em" }}>
                          TODAY
                        </span>
                      )}
                    </button>
              ...
              <div className="sfont" style={{ fontSize: 12, color: "#888", marginTop: 10, letterSpacing: "0.04em" }}>
                ...
                {" · "}
                <span style={{ color: loading ? "#888" : availableCount > 3 ? "#2d7a52" : "#c0392b", fontWeight: 500 }}>
                  {loading ? "Loading…" : `${availableCount} slots available`}
                </span>
              </div>
            </div>
            ...
            <div style={{ marginBottom: 28 }}>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
                {[30, 60, 90].map(d => (
                  <button key={d}
                    ...
                    style={{
                      flex: 1, padding: "10px 8px", border: "1.5px solid #1c1c1c",
                      background: duration === d ? "#1c1c1c" : "transparent",
                      color: duration === d ? "#f5f2ed" : "#1c1c1c",
              ...
              {loading ? (
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 6 }}>
                  {slots.map(s => (
                    <button key={s.id} disabled={s.is_booked}
                      className={`slot-btn${s.is_booked ? " taken" : ""}${selectedSlot?.id === s.id ? " selected" : ""}`}
                      onClick={() => !s.is_booked && setSelectedSlot(selectedSlot?.id === s.id ? null : s)}>
                      {toAmPm(s.time)}
                    </button>
                  ))}
                </div>
              ...
              <div className="sfont" style={{ display: "flex", gap: 16, marginTop: 12, fontSize: 11, color: "#888" }}>
                {[
                  ...
                  { bg: "#e8e5e0",     border: "none",                 label: "Booked"    },
                  { bg: "#2d7a52",     border: "none",                 label: "Selected"  },
                ].map(l => (
                  <span key={l.label} style={{ display: "flex", alignItems: "center", gap: 5 }}>
                    <span style={{ width: 12, height: 12, background: l.bg,
                      border: l.border, display: "inline-block" }} />
                    {l.label}
                  </span>
                ))}
              </div>
            ...
            {/* Selected summary */}
            {selectedSlot && (
              <div style={{ border: "1px solid #2d7a52", padding: "14px 18px",
                marginBottom: 16, background: "#f0faf4" }}>
                <div className="sfont" style={{ fontSize: 12, color: "#2d7a52",
                  letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 4 }}>
                  Selected
                </div>
                <div className="pfont" style={{ fontSize: 20 }}>
                  {selectedSlot.time} — {nextSlot(selectedSlot.time)}
                </div>
                <div className="sfont" style={{ fontSize: 13, color: "#555", marginTop: 4 }}>
                  {MONTHS[selectedDate.getMonth()]} {selectedDate.getDate()} · ${court.price_per_hour ?? "—"}.00
                </div>
              </div>
            )}
      ...
      {confirmOpen && selectedSlot && (
        <div className="modal-backdrop" onClick={() => setConfirmOpen(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <p className="sfont" style={{ fontSize: 14, color: "#666", marginBottom: 28 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {[
                ...
                { label: "Time",     value: `${selectedSlot.time} — ${nextSlot(selectedSlot.time)}` },
                { label: "Duration", value: `${selectedSlot.duration_minutes} min` },
                { label: "Total",    value: `$${court.price_per_hour ?? "—"}.00` },
              ].map(r => (
                <div key={r.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                  <span className="sfont" style={{ fontSize: 12, color: "#888",
                    letterSpacing: "0.08em", textTransform: "uppercase" }}>{r.label}</span>
            ...
            <div className="divider" />
            {bookingError && (
              <p className="sfont" style={{ fontSize: 13, color: "#c0392b", marginBottom: 16 }}>
                {bookingError}
              </p>
            )}
            <div style={{ display: "flex", gap: 12 }}>
              <button className="btn-outline" style={{ flex: 1 }} onClick={() => setConfirmOpen(false)}>
                Cancel
              </button>
      ...
      {booked && (
        <div className="toast">
          ✓ Booking confirmed for {selectedSlot?.time}!
        </div>
      )}
    </div>
  );
